...
export class AIAnalysisService {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a professional climbing coach and performance analyst. Analyze the climbing session data and provide specific, actionable feedback. Format your response with clear sections: SUMMARY, STRENGTHS, AREAS FOR IMPROVEMENT, RECOMMENDATIONS, and PROGRESS INSIGHTS. Use bullet points for lists.",
            },
            {
              role: "user",
              content: prompt,
  ...
  private generateAnalysisPrompt(session: Session): string {
    ...
    const attempts = session.climbs.filter(
      (c) => c.tickType === "attempt",
    ).length;
    const flashes = session.climbs.filter((c) => c.tickType === "flash").length;
    const onsights = session.climbs.filter(
      (c) => c.tickType === "onsight",
    ).length;

    const grades = session.climbs.map((c) => c.grade);
    const efforts = session.climbs
      .map((c) => c.effort)
...
[1-2 sentences about patterns and progress indicators from this session]`;
  ...
  private parseAnalysis(analysisText: string): AnalysisResult {
    ...
    const sections = {
      summary: this.extractSection(analysisText, [
        "SUMMARY",
        "Session Summary",
        "Overview",
      ]),
      strengths: this.extractListSection(analysisText, [
        "STRENGTHS",
        "Strengths",
        "What went well",
      ]),
      areasForImprovement: this.extractListSection(analysisText, [
        "AREAS FOR IMPROVEMENT",
        "Areas for Improvement",
        "Improvement Areas",
        "Areas to improve",
      ]),
      recommendations: this.extractListSection(analysisText, [
        "RECOMMENDATIONS",
        "Recommendations",
        "Suggestions",
      ]),
      progressInsights: this.extractSection(analysisText, [
        "PROGRESS INSIGHTS",
        "Progress Insights",
        "Progress",
        "Insights",
    ...
    const result = {
      summary:
        sections.summary || "Session completed with good overall performance.",
      strengths: sections.strengths,
      areasForImprovement: sections.areasForImprovement,
      recommendations: sections.recommendations,
      progressInsights:
        sections.progressInsights ||
        "Continue building on current performance and addressing areas for growth.",
    };
