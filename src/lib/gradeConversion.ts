...
const ydsToFrenchMap: Record<string, string> = {
  ...
  '5.11a': '6c', '5.11b': '6c', '5.11c': '6c+', '5.11d': '7a',
...
const frenchToYdsMap: Record<string, string> = Object.fromEntries(
  Object.entries(ydsToFrenchMap).map(([yds, fr]) => [fr.toUpperCase(), yds]) // Store French grades uppercase for mapping
);
// Also add lowercase french grades for broader compatibility
Object.entries(ydsToFrenchMap).forEach(([yds, fr]) => {
    if (fr !== fr.toUpperCase()) frenchToYdsMap[fr] = yds;
});


const vScaleToFontMap: Record<string, string> = {
  'VB': '3', 'V0': '4', 'V0+': '4+',
  'V1': '5', 'V2': '5+', 'V3': '6A', 'V3+': '6A+',
  'V4': '6B', 'V4+': '6B+', 'V5': '6C', 'V5+': '6C+',
  'V6': '7A', 'V7': '7A+', 'V8': '7B', 'V8+': '7B+',
  'V9': '7C', 'V10': '7C+', 'V11': '8A', 'V12': '8A+',
  'V13': '8B', 'V14': '8B+', 'V15': '8C', 'V16': '8C+', 'V17': '9A',
};

const fontToVScaleMap: Record<string, string> = Object.fromEntries(
  Object.entries(vScaleToFontMap).map(([v, font]) => [font.toUpperCase(), v])
);
// Add variations for Font grades (e.g. 6a, 6A+, 6a+)
Object.entries(vScaleToFontMap).forEach(([v, font]) => {
    const upperFont = font.toUpperCase();
    if (font !== upperFont) fontToVScaleMap[font] = v; // e.g. 6a
    if (upperFont.endsWith('+') && upperFont.length > 1) { // e.g. 6A+
        const base = upperFont.slice(0, -1); // 6A
        if (base !== base.toLowerCase()) fontToVScaleMap[base.toLowerCase() + '+'] = v; // 6a+
    }
});
...
/**
 * Tries to detect the grade system from a grade string.
 * This is very basic and unreliable for ambiguous grades without climb type context.
 * @param grade The grade string.
 * @param isBoulderHint Optional hint if the climb is known to be bouldering.
 * @returns A guessed GradeSystem or undefined.
 */
export function detectGradeSystem(grade: string, isBoulderHint?: boolean): GradeSystem | undefined {
  if (!grade) return undefined;
  const upperGrade = grade.toUpperCase();

  // V-Scale check (most specific)
  if (upperGrade.startsWith('V') && (upperGrade === 'VB' || (parseInt(upperGrade.substring(1)) >= 0 && parseInt(upperGrade.substring(1)) <= 17))) return GradeSystem.VSCALE;

  // YDS check
  if (upperGrade.startsWith('5.')) return GradeSystem.YDS;

  // Font / French (can be ambiguous, e.g., "6A", "7B+")
  // Regex for typical Font/French: digit, optional A/B/C, optional +
  // e.g., 4, 5+, 6A, 6a, 7B+, 7b+
  if (upperGrade.match(/^[3-9][ABC]?\+?$/i)) {
    if (isBoulderHint === true) return GradeSystem.FONT;
    if (isBoulderHint === false) return GradeSystem.FRENCH;
    // If no hint, it's ambiguous. Could try to see if it exists in one map but not the other.
    // For now, prefer Font if it looks like a Font grade, else French.
    if (fontToVScaleMap[upperGrade]) return GradeSystem.FONT;
    if (frenchToYdsMap[upperGrade]) return GradeSystem.FRENCH;
    // Default to Font for single digits or uppercase with A/B/C if no other info
    if (upperGrade.match(/^[3-9]$/) || upperGrade.match(/^[6-9][ABC]\+?$/)) return GradeSystem.FONT;
...
/**
 * Normalizes a grade to a target system.
 * Default target for routes is French, for boulders is Font.
 */
export function normalizeGrade(
  grade: string,
  originalSystem?: GradeSystem,
  ...
  let detectedOriginalSystem = originalSystem || detectGradeSystem(grade, isBoulder);
  let effectiveTargetSystem = targetSystem;

  if (!effectiveTargetSystem) {
    effectiveTargetSystem = isBoulder ? GradeSystem.FONT : GradeSystem.FRENCH;
  }

  // If original system couldn't be detected, return original grade.
  // Further processing relies on knowing the source system.
  if (!detectedOriginalSystem) {
    // console.warn(`normalizeGrade: Original system for grade "${grade}" is unknown. Cannot convert.`);
    return grade;
  }

  const gradeToConvert = grade; // Keep original casing for French map if needed, use upper for V-Scale

  try {
    if (detectedOriginalSystem === effectiveTargetSystem) return gradeToConvert;

    // YDS -> French
    if (detectedOriginalSystem === GradeSystem.YDS && effectiveTargetSystem === GradeSystem.FRENCH) {
      return ydsToFrenchMap[gradeToConvert] || gradeToConvert;
    }
    // French -> YDS
    if (detectedOriginalSystem === GradeSystem.FRENCH && effectiveTargetSystem === GradeSystem.YDS) {
      return frenchToYdsMap[gradeToConvert.toUpperCase()] || frenchToYdsMap[gradeToConvert.toLowerCase()] || gradeToConvert;
    }

    // V-Scale -> Font
    if (detectedOriginalSystem === GradeSystem.VSCALE && effectiveTargetSystem === GradeSystem.FONT) {
      return vScaleToFontMap[gradeToConvert.toUpperCase()] || gradeToConvert;
    }
    // Font -> V-Scale
    if (detectedOriginalSystem === GradeSystem.FONT && effectiveTargetSystem === GradeSystem.VSCALE) {
      return fontToVScaleMap[gradeToConvert.toUpperCase()] || fontToVScaleMap[gradeToConvert.toLowerCase()] || gradeToConvert;
    }

    // YDS -> V-Scale (Not directly possible, boulder vs route)
    // YDS -> Font (Not directly possible)
    // French -> V-Scale (Not directly possible)
    // French -> Font (Not directly possible)
    // etc.

    // console.warn(`normalizeGrade: Conversion from ${detectedOriginalSystem} to ${effectiveTargetSystem} for grade "${gradeToConvert}" is not supported yet.`);
  } catch (error) {
    // console.error(`normalizeGrade: Error during conversion of "${gradeToConvert}" from ${originalSystem} to ${targetSystem}:`, error);
  }
  return gradeToConvert; // Return original if no conversion path found or error
}
